...
include( ${CMAKE_CURRENT_LIST_DIR}/eval_genex.cmake )
...
function( get_linked_target_properties out_var)
    ...
    foreach( target IN LISTS targets )
        ...
        foreach( link_target IN LISTS link_libs interface_link_libs )
            ...
            if ( link_target MATCHES "^[$][<]GENEX_EVAL:.+[>]$" )
                # Try to evaluate the generator-expression now.
                eval_genex(evaluated "${link_target}")
                list( APPEND other_targets ${evaluated} )
            elseif ( link_target MATCHES "^[$][<]LINK_ONLY:(.+)[>]$" )
        ...
        foreach( other_target IN LISTS other_targets )
            if (NOT TARGET ${other_target})
                if ("${other_target}" MATCHES "^[$][<].+[>]$")
                    message( WARNING "Not a (valid) target (because I cannot evaluate this generator-expressions): ${other_target}" )
                ...
                elseif (${other_target} MATCHES "^[^:]+$")
                    message( DEBUG "Not a target (with namespace syntax), but possibly a library to link: ${other_target}" )
                else()
